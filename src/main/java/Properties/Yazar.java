package Properties;

public class Yazar {
	private String yazarAdi;
	private String yazarYazi;
	private String yazarGrup;
	private String yazarResimUrl;
	private String yazarId;
	private String yazarResimKlasor;
	private String yazarYaziId;
	public String getYazarResimKlasor() {
		return yazarResimKlasor;
	}
	public void setYazarResimKlasor(String yazarResimKlasor) {
		this.yazarResimKlasor = yazarResimKlasor;
	}
	public String getYazarId() {
		return yazarId;
	}
	public void setYazarId(String yazarId) {
		this.yazarId = yazarId;
	}
	public String getYazarResimUrl() {
		return yazarResimUrl;
	}
	public void setYazarResimUrl(String yazarResimUrl) {
		this.yazarResimUrl = yazarResimUrl;
	}
	public String getYazarGrup() {
		return yazarGrup;
	}
	public void setYazarGrup(String yazarGrup) {
		this.yazarGrup = yazarGrup;
	}
	public String getYazarAdi() {
		return yazarAdi;
	}
	public void setYazarAdi(String yazarAdi) {
		this.yazarAdi = yazarAdi;
	}
	public String getYazarYazi() {
		return yazarYazi;
	}
	public void setYazarYazi(String yazarYazi) {
		this.yazarYazi = yazarYazi;
	}
	public String getYazarYaziId() {
		return yazarYaziId;
	}
	public void setYazarYaziId(String yazarYaziId) {
		this.yazarYaziId = yazarYaziId;
	}

}
